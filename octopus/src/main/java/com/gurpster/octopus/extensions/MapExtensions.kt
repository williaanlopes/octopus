...
fun <K, V> MutableMap<K, V>.putIfNotNull(value: Pair<K, V?>): V? =
    putIfNotNull(value.first, value.second)

fun <K, V> MutableMap<K, V>.putIfNotNull(key: K, value: V?): V? =
    if (value != null) put(key, value) else get(key)

fun <K, V> MutableMap<K, V>.putAllIfNotNull(map: Map<K, V>?) = map?.let { putAll(it) } ?: Unit

fun <K, V> MutableMap<K, V>.putIfNotContains(value: Pair<K, V>): V? =
    putIfNotContains(value.first, value.second)

fun <K, V> MutableMap<K, V>.putIfNotContains(key: K, value: V): V? =
    if (containsKey(key)) get(key) else put(key, value)
